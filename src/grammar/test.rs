...
use crate::grammar::*;
    ...
    fn test_extended_nonterminals(){
        let pred = extended_nonterminals(
            &Union::OnlyA("<foo>* <bar>+ <elem>? <none>".to_string()));
            let answer  = vec!["<foo>*".to_string(), "<bar>+".to_string(), "<elem>?".to_string()];
            assert_eq!(pred.len(), answer.len());
            for i in 0.. pred.len(){
                assert_eq!(pred[i], answer[i]);
            }
    ...
    fn test_new_symbol(){
        ...
        grammars.insert("<start>".to_string(), vec![Union::OnlyA("<expr>".to_string())]);
        grammars.insert(
            "<expr>".to_string(),
        ...
        grammars.insert("<integer>".to_string(), vec![Union::OnlyA("<digit><integer>".to_string()), Union::OnlyA("<digit>".to_string())]);
        grammars.insert(
            "<digit>".to_string(),
